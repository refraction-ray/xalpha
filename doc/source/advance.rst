...
.. note::
    ...
    ⚠️ **云端环境兼容性警告**：由于聚宽（JoinQuant）云端平台研究环境目前仅支持非常老版本的 Python（如 Python 3.6），而现代版本的 xalpha 依赖较新的 Python 3.10+ 和 Pandas 特性，因此直接在聚宽云平台安装最新版 xalpha 很有可能发生库冲突或不兼容。如果在此环境使用受阻，强烈建议采用第一种方案（本地安装 xalpha 并通过 `jqdatasdk` 在本地调用聚宽数据）。
...
.. code-block:: python
    ...
    # 以下为历史数据（例如2020年）示例，实际使用请根据所需年份更新
    no_trading_days["LU"] = [
        "2020-01-01",
        "2020-04-10",
        "2020-04-13",
